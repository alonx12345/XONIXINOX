...
namespace BayatGames.SaveGameFree
{
    ...
    public static class SaveGame
    {
        ...
        private static SaveGamePath m_SavePath = SaveGamePath.Streaming;
            ...
            if (!IsFilePath(identifier))
            {
                switch (path)
                {
                    ...
                    case SaveGamePath.DataPath:
                        ...
                        break;
                    case SaveGamePath.Streaming:
                        if (Application.platform == RuntimePlatform.Android)
                        {
                            filePath = string.Format("{0}/{1}", "jar: file://" + Application.dataPath + "!/assets/", identifier);
                        }
                        else
                        {
                            filePath = string.Format("{0}/{1}", Application.streamingAssetsPath, identifier);
                        }
                        break;
                    default:
                        break;
